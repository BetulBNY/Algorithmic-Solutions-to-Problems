...
# Inputs:
n1 = 2
logs1 = ["0:start:0","1:start:2","1:end:5","0:end:6"] # Output: [3,4]

n2 = 1
logs2 = ["0:start:0","0:start:2","0:end:5","0:start:6","0:end:6","0:end:7"] # Output: [8]

n3 = 2
logs3 = ["0:start:0","0:start:2","0:end:5","1:start:6","1:end:6","0:end:7"] # Output: [7,1]
...
def exclusiveTime(n, logs):
    ...
    for log in logs:
        ...
        time = int(time)
        if typ == "start":
            if stack: # Same as: if len(stack) > 0:
            ...
            result[finished] += time - prev_time +1
            prev_time = time + 1
    return result

print(exclusiveTime(n1, logs1))
